...
M <- matrix(
        data = c(1, 2, 3, 4, 5, 6), # data elements
...
# we can use segments() and lines() functions to add segments and lines to an existing plot:
...
segments(
        ...
        x1 = c(0, 10, 10, 0, 10), y1 = c(10, 10, 0, 0, 10),
        lwd = 2, col = 2, lty = 2
)
...
plot(x1, y1,
        col = 1:10, pch = 1:10, cex = 1:10, lwd = 3,
...
x3 <- seq(-5, 5, by = 0.1)
plot(sin(x3), type = "l", ylab = "", xlab = "x", lwd = 2, col = "red")

lines(cos(x3), lwd = 2, col = "blue")
...
plot(density(x4, bw = 0.1)) # less smoothing
...
# A class can be assigned to an object by just adding an attribute to it
...
# Define the custom S3 object
the_person <- list(name = "John", age = 25)
class(the_person) <- "Person"

# Define the print method for the 'person' class
